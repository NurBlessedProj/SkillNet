...
import { CreateUser } from "@/app/apis/auth/createUser";
...
export default function RegisterRecruiter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password === confirmPassword) {
      try {
        await Createuser.Create(email, password, "admin");
      } catch (err) {
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                >
                  {Createuser.loading ? (
                    ...
                    <>
                      <span className="text-white">{t("Register.signUp")}</span>
                ...
                {Createuser.error && (
                  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md p-4">
                    <p className="text-red-600 dark:text-red-400 text-sm text-center">
                      {t("Register.error", { error: Createuser.error })}
                    </p>
                  </div>
                )}
              ...
              <div className="space-y-3 md:space-y-4">
                <div className="flex items-center space-x-2 md:space-x-3">
                  ...
                  <span className="text-sm md:text-base text-emerald-100">
                    {t("Register.recruiterFeatures.postJobs")}
                  </span>
                </div>
                <div className="flex items-center space-x-2 md:space-x-3">
                  <div className="w-1.5 md:w-2 h-1.5 md:h-2 bg-emerald-300 rounded-full"></div>
                  <span className="text-sm md:text-base text-emerald-100">
                    {t("Register.recruiterFeatures.findCandidates")}
                  </span>
                </div>
                <div className="flex items-center space-x-2 md:space-x-3">
                  <div className="w-1.5 md:w-2 h-1.5 md:h-2 bg-emerald-300 rounded-full"></div>
                  <span className="text-sm md:text-base text-emerald-100">
                    {t("Register.recruiterFeatures.reviewAssessments")}
                  </span>
                </div>
                <div className="flex items-center space-x-2 md:space-x-3">
                  <div className="w-1.5 md:w-2 h-1.5 md:h-2 bg-emerald-300 rounded-full"></div>
                  <span className="text-sm md:text-base text-emerald-100">
                    {t("Register.recruiterFeatures.buildPipeline")}
                  </span>
                </div>
              </div>
            </div>
